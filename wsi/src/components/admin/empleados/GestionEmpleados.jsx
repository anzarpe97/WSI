...
const GestionEmpleados = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      // 1. Consultar todos los mantenimientos y filtrar por id_mecanico y estado ACTIVO
      const token = localStorage.getItem('token');
      const resp = await fetch(`http://localhost:8000/api/mantenimientos/`, {
        headers: {
          'Authorization': `Token ${token}`,
        },
      });
      if (!resp.ok) throw new Error('Error al verificar mantenimientos activos');
      const data = await resp.json();
      const mantenimientos = data.results || data;
      // Filtrar por id_mecanico y estado ACTIVO
      const activos = mantenimientos.filter(m => {
        const idMecanico = m.id_mecanico && (m.id_mecanico.id || m.id_mecanico);
        return idMecanico === empleadoToDelete.id && (m.estado || '').toUpperCase() === 'ACTIVO';
      });
      // Mostrar en consola cómo se está validando
      console.log('[VALIDACIÓN] Empleado:', empleadoToDelete.nombre, empleadoToDelete.apellido, 'Mantenimientos:', mantenimientos, 'Activos:', activos);
      if (activos.length > 0) {
        setShowDeleteModal(false);
        setShowActiveMaintModal(true);
        return;
      }
      // 2. Si no tiene mantenimientos activos, proceder con el borrado
                  ...
                  <tr key={empleado.id}>
                    ...
                    <td data-label="Acciones">
                      <div className="empleados-acciones">
                        ...
                        {showActiveMaintModal && empleadoToDelete && (
                          <div className="modal-overlay">
                            <div className="modal-content" style={{ maxWidth: 420, borderRadius: 10, padding: '1.5rem', boxShadow: '0 5px 15px rgba(0,0,0,0.3)', background: 'white', textAlign: 'center' }}>
                              <h2 style={{ fontSize: '1.3rem', fontWeight: 'bold', color: '#ff6a00', marginBottom: '1rem' }}>No se puede eliminar el empleado</h2>
                              <p style={{ fontSize: '1rem', color: '#b03800', marginBottom: '1.2rem', fontWeight: 500 }}>
                                Este empleado tiene mantenimientos activos asociados.<br />
                                Debe finalizar o reasignar los mantenimientos antes de poder eliminarlo.
                              </p>
                              <div className="modal-actions" style={{ display: 'flex', justifyContent: 'center', marginTop: '1.2rem' }}>
                                <button onClick={() => setShowActiveMaintModal(false)} className="btn btn-confirmar" style={{ backgroundColor: '#ff6a00', color: 'white', border: 'none', borderRadius: 20, padding: '0.6rem 1.2rem', fontSize: '0.9rem', fontWeight: 600, cursor: 'pointer', transition: 'background-color 0.2s, transform 0.2s' }}>Entendido</button>
                              </div>
                            </div>
                          </div>
                        )}
